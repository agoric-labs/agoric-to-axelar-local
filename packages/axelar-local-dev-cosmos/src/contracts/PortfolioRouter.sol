...
contract PortfolioRouter is AxelarExecutable, RemoteRepresentative, IPortfolioRouter {
    ...
    function processInstruction(RouterInstruction calldata instruction) external {
        ...
        if (instruction.depositPermit.length > 0) {
            ...
            // Permit2 transfer happens before provide/multicall, but this is safe:
            // processInstruction runs inside a try/catch in _execute, so if any
            // subsequent step (provide or multicall) reverts, the entire instruction
            // — including the permit2 transfer — is rolled back atomically.
