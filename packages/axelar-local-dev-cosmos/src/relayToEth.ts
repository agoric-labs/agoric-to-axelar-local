...
export const relayDataToEth = async () => {
  ...
  console.log(ethereumNetwork.gateway.address);
  const q = encode(["address"], [ethereumNetwork.gateway.address]);
  const p = decode(["address"], Buffer.from([
    0,
    0,
    0,
    0,
  ...
  const actualMessage = encode(["string"], ["fraz"]);
  const payload2 = encode(
    ["string", "string", "bytes"],
    ["Ethereum", ethereumContract.address, actualMessage]
  );
  const payload3 = encoder.encode(
  ...
  const ethPayload = encodeVersionedPayload(1, payload3);
  await gatewayContract.callContract(
    "agoric",
    ethereumContract.address,
    Array.from(ethPayload)
  );
  ...
  console.log(
    "Stake Token Balance:",
    (await stakeContract.balanceOf(wallet.address)).toString()
  );
  console.log(
    "USDC Balance:",
    (await tokenContract.balanceOf(wallet.address)).toString()
  );
  console.log(
    "stake contract USDC Balance:",
    (await tokenContract.balanceOf(stakeContract.address)).toString()
  );
};
